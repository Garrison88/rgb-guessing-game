...
// populate squares on page load
function init() {
	for (var i = 0; i < gameModeBtn.length; i++) {
		gameModeBtn[i].addEventListener("click", function() {
...
function pickColor() {
	...
	correctNum = random;
	pickedColorHEX = rgbToHex(
		colors[random].match(/\d+/g)[0],
		colors[random].match(/\d+/g)[1],
		colors[random].match(/\d+/g)[2]
		);
...
function changeColors(color) {
	//loop through squares and change color to 'color'
	for (var i = 0; i < squares.length; i++) {
...
function reset() {
	...
	pickedColorRGB = pickColor();
	colorDisplay.textContent = pickedColorRGB;
	newButton.textContent = "New Colors [N]";
	message.textContent = "";
	gameOver = false;
	for (var i = 0; i < squares.length; i++) {
		if (colors[i]) {
...
//these two functions convert pickedColorRGB to pickedColorHEX in order to
